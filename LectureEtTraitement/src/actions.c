...
//Fonction composant le tri Fusion :
void fusion (Informations *T, int deb1, int fin1, int fin2, int mode)
{
    Informations *table1; //Instance d'un tableau temporaire
    int deb2 = fin1+1;
    int compt1 = deb1;
    int compt2 = deb2;
    int i;

    table1 = malloc((fin1-deb1+1)*sizeof(Information)); //Allocation de mémoire au tabeau temporaire

    for(i = deb1; i <= fin1 ; i++)
    {
        (*table1)[i-deb1] = (*T)[i]; //On attribu les données de la partie du tableau dans le tableau temporaire
    }

    for(i = deb1 ; i <= fin2; i++)
    {
        if(mode == 1) //Si on veut classer le tableau par ordre de poul
        {
            if(compt1 == deb2) // Lorsque l'index du tableau temporaire est égale à celui du 1er élément de la deuxième partie de tableau
            {
                break; //Sortir
            }
            else if (compt2 == (fin2+1))
            {
                (*T)[i] = (*table1)[compt1-deb1]; //On attribu la ligne du tableau temporaire à la partie de tableau
                compt1++; //On incrémente l'index du tableau temporaire
            }
            else if((*table1)[compt1-deb1].poul < (*T)[compt2].poul) //On compare si l'index i de la partie de tableau est supérieur à celui du tableau temporaire
            {
    ...
    free(table1); //Libération de la mémoire allouée au tableau temporaire
}
...
{
    if(deb!=fin) //Si la partie du tableau est composée de plus d'une ligne
    {
        int milieu = (fin+deb)/2; //On divise cette partie en deux
        triFusionBis(T, deb, milieu, mode); //On réexecute la fonction avec les deux parties
        triFusionBis(T, milieu+1, fin, mode);
        fusion(T, deb, milieu, fin, mode); //On lance la fusion des parties de tableau
    }
}
...
{
    if(longueur > 0) //Si le tableau contient plus d'un élément
    {
