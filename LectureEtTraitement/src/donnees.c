...
int tailleDonnees() //Fonction permettant de recueillir le nombre total de ligne du fichier CSV
{
    ...
    if(fichierCSV != NULL) //Si l'ouverture à réussie :
    {
        ...
        while(fgets(chaine, sizeof(chaine), fichierCSV) != NULL)
        {
            if(chaine[0] != '\n')
            {
                i++; //Incrémentation de i pour chaque ligne trouvée
            }
        }
...
void extraireDonnees(Informations *tableau, int nbLigne) //Fonction permettant de mettre en mémoire les données du fichier CSV
{
    ...
    if(fichierCSV != NULL)
    {
        for (int i = 0; i < nbLigne; i++)
        {
            ...
            fscanf(fichierCSV, "%d;%d", &(*tableau)[i].poul, &(*tableau)[i].temps); //Pour chaque ligne, on prend les valeurs du fichier CSV ("temps;poul") pour les mettre en mémoire (tableau de structure)
